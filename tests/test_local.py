...
from .base import local_vault, local_app
...
async def test_vault_metadata(local_vault):
    app = self.app
    await app.initialize()
    await app.open_or_init(local_vault)
    backend = local_vault.backend
...
async def test_revision_increase_after_push(local_vault):
    app = SyncryptApp(self.app_config)
    await app.add_vault(local_vault)
    await app.initialize()
    await app.open_or_init(local_vault)
...
async def test_two_local_one_remote(local_vault):
    ...
    self.assertSameFilesInFolder(local_vault.folder, other_vault_path)

    keys = UserVaultKeyManager(self.app)
    # We have one valid key for both vaults
    assert len(keys.list_for_vault(self.other_vault)) == 1
    ...
    self.assertNotEqual(key.fingerprint, local_vault.identity.get_fingerprint())
    assert key.fingerprint == self.app.identity.get_fingerprint()

async def test_local_metadata(local_vault):
    other_vault_path = os.path.join(VaultLocalTestCase.working_dir, "othervault")
    ...
    revision = await local_vault.backend.set_vault_metadata(self.app.identity)
    ...
    assert self.other_vault.revision_count == original_count + 1

    self.assertEqual(
        local_vault.config.get("vault.name"), self.other_vault.config.get("vault.name")
    )

async def test_remove_file(local_vault):
    other_vault_path = os.path.join(VaultLocalTestCase.working_dir, "othervault")
    ...
    self.assertNotEqual(pre_rev, local_vault.revision)
...
async def test_local_fake_revision(local_vault):
    other_vault_path = os.path.join(VaultLocalTestCase.working_dir, "othervault")
    # remove "other vault" folder first
    if os.path.exists(other_vault_path):
        shutil.rmtree(other_vault_path)
...
async def test_local_full_pull(local_vault):
    app = self.app
    await self.app.initialize()
    await app.add_vault(local_vault)
    await app.open_or_init(local_vault)
...
async def test_add_and_remove_user(local_vault):
    app = SyncryptApp(self.app_config)
    await app.add_vault(local_vault)
    await app.initialize()
    await app.open_or_init(local_vault)
...
async def test_add_user_with_a_key(local_vault):
    app = SyncryptApp(self.app_config)
    await app.add_vault(local_vault)
    await app.initialize()
    await app.open_or_init(local_vault)
...
async def test_add_user_twice(local_vault):
    app = SyncryptApp(self.app_config)
    await app.add_vault(local_vault)
    await app.initialize()
    await app.open_or_init(local_vault)
