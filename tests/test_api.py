...
class APITests(VaultTestCase):
    ...
    def test_api_login(self):
        ...
        try:
            ...
            r = yield from aiohttp.get(TEST_API_ENDPOINT + '/v1/auth/check/')
            c = yield from r.json()
            yield from r.release()
            self.assertEqual(r.status, 200)
            self.assertEqual(c['connected'], True)

            r = yield from aiohttp.get(TEST_API_ENDPOINT + '/v1/auth/logout/')
            c = yield from r.json()
            self.assertEqual(r.status, 200)
            yield from r.release()
    ...
    def test_api_bundle(self):
        ...
        try:
            ...
            bundle_list_uri = TEST_API_ENDPOINT + '%sbundle/' % vault_uri
    ...
    def test_api_add_user(self):
        ...
        try:
            ...
            r = yield from aiohttp.post(TEST_API_ENDPOINT + '/v1/vault/',
            ...
            r = yield from aiohttp.get(TEST_API_ENDPOINT + teh_vault + 'users/')
            c = yield from r.json()
            ...
            r = yield from aiohttp.get(TEST_API_ENDPOINT + teh_vault + 'users/' + me['email'] + '/keys/')
            c = yield from r.json()
            self.assertGreater(len(c), 0) # at least one key
            yield from r.release()
    ...
    def test_api_watchdog(self):
        ...
        try:
            ...
            r = yield from aiohttp.get(TEST_API_ENDPOINT + '/v1/config')
            c = yield from r.json()
